...
These two projects use gradle with the `idea` plugin. So you can easily import them with IntelliJ.
...
- <dfn>KeyNode</dfn>: A KeyNode is basically an identifier in the terminology of a connector. KeyNodes are not stored in
  the backend.
- <dfn>KeyNode</dfn>: In the past they simply were named ContainerNodes and others were basically DocumentNodes. This
  is actually the term which is still being used in the Connector-SDK implementation.
